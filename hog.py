...
def is_always_roll(strategy, goal=GOAL):
    ...
    for score1 in range(goal):
        for score2 in range(goal):
            if strategy(score1, score2) != botton:
                return False
    return True
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    num_of_roll = 1
    for i in range(1, 11):
        candidate = averaged_roll_dice(i, dice)
        if candidate> top:
            num_of_roll = i
            top = candidate
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    if score + boar_brawl(score, opponent_score) >=GOAL:
        return 0
    if score + take_turn(1, score, opponent_score, six_sided):
        return 1
    if score + take_turn(2, score, opponent_score, six_sided):
        return 2
    if score > opponent_score:
        return 3
    else:
...
def boar_vs_roll6_average(score, opponent_score):
    boar_score = boar_strategy(score, opponent_score)
    averaged_roll_dice = make_averaged(roll_dice, times_called=1000)
    average_six_dice = averaged_roll_dice(6, six_sided)     
    if boar_score>= average_six_dice:
        return 0
    else:
        return 6
# END PROBLEM 12
